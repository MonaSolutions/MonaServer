...
ADD_TEST(StringTest, TestMultiple) {
	...
	CHECK(String::Format(_Str, big) == "1.234567890123457e+018");
	
	float bigf = (float)1234567890123456789.1234567890;
	CHECK(String::Format(_Str, bigf) == "1.2345679e+018");

	// TODO
	//CHECK(ToNumber<float>(_Str, bigf));
}

ADD_TEST(StringTest, TestToNumber) {

	CHECK(ToNumber<int>("123", 123));
	CHECK(ToNumber<int>("-123", -123));
	CHECK(ToNumber<UInt64>("123", 123));
	CHECK(!ToNumber<UInt64>("-123", 123));
	CHECK(ToNumber<double>("12.34", 12.34));
	CHECK(ToNumber<float>("12.34", 12.34f));

	// Test of delta epsilon precision
	CHECK(ToNumber<double>("0", DBL_EPSILON));
	CHECK(ToNumber<float>("0", FLT_EPSILON));
	CHECK(!ToNumber<double>("0", DBL_EPSILON * 2));
	CHECK(!ToNumber<float>("0", FLT_EPSILON * 2));

	CHECK(!ToNumber<double>("", 0));
	CHECK(!ToNumber<double>("asd", 0));
	CHECK(!ToNumber<double>("a123", 0));
	CHECK(!ToNumber<double>("a123", 0));
	CHECK(!ToNumber<double>("z23", 0));
	CHECK(!ToNumber<double>("23z", 0));
	CHECK(!ToNumber<double>("a12.3", 0));
	CHECK(!ToNumber<double>("12.3aa", 0));
}
